...
export default function Order() {
  ...
  return (
    <main className="bg-lightGreen dark:bg-black min-h-screen flex justify-center pt-40 pb-28 clip-custom">
      <div className="wrapper-common flex-col items-center gap-8">
        ...
        {items.map((item) => (
          <div key={item.id} className="flex items-center justify-between text-lg bg-white dark:bg-black w-full px-6 py-10 border border-[#35B8BE26] dark:border-white rounded-lg">
            <section className="flex gap-6">
              <img src={item.image} className="size-32" />
              <h2 className="mt-8 dark:text-white">{item.title}</h2>
            </section>
            <section className="h-full flex items-center gap-6">
              <h3 className="text-green mr-10">$ {(item.price * item.quantity).toFixed(2)} USD</h3>
              <div className="bg-[#FAFAFA] dark:bg-black dark:text-white border border-[#DDDDDD] px-6 py-2 rounded-lg">{item.quantity}</div>
              <button
